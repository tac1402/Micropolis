...
namespace TAC
{
		...
		where T : struct
	{
		...
		public DiscreteMap_(Vector3 argCenter, Vector2Int argHalfSize, float argDiscreteStep, bool argNoCenter) :
			this(argCenter, argHalfSize, argDiscreteStep, -1)
		{ }
		...
		where T : class
	{
		...
		public DiscreteMap(Vector3 argCenter, Vector2Int argHalfSize, float argDiscreteStep, bool argNoCenter) :
			this(argCenter, argHalfSize, argDiscreteStep, -1)
		{ }
	...
	public class DiscreteMap
	{
		...
		{ 
			...
			if (argNumber == 0)
			{
				CreateFull(argHalfSize, argDiscreteStep);
			}
			else if (argNumber == -1)
			{
				CreateNoCenter(argHalfSize, argDiscreteStep);
			}
			else
			{
				Create(argHalfSize, argDiscreteStep, argNumber);
			}
		}

		private void CreateNoCenter(Vector2Int argHalfSize, float argDiscreteStep)
		{
			...
			for (int i = 1; i < argHalfSize.x + 1; i++)
			{
				...
				for (int j = 1; j < argHalfSize.y + 1; j++)
				{
					...
					if (xm != argHalfSize.x * -1 && ym != argHalfSize.y * -1)
					{
